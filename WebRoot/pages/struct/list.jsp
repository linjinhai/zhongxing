 ...
 <c:if test="${ judge eq 'no'}">
		...
		<div class="page-header">
			<ol class="breadcrumb">
				...
				<li><a href="#" onclick="javascript:crumb('struct/list');">评定项管理</a></li>
				<li class="active">评定项列表</li>
			</ol>
		</div>
							...
							<div class="navbar-form navbar-left">
								<div class="form-group">
									...
									<span class="input-group-addon"><font style="color: white;">所在方位</font></span>
										...
										<div class="input-group input-group-no-border">
										<input type="text" id="search" name="search" value="${pager.search}" placeholder="评定项名称..." class="form-control form-heading">
										<span class="input-group-addon"><a href="javascript:pager('1');">搜索</a></span>
									</div>
								</div>
							</div>
							<div class="btn-group-sm pull-right" role="group">
								<button type="button" class="btn btn-info btn-xs" onclick="javascript:add('struct/insertPage','1');"><span class="glyphicon glyphicon-plus"></span> 添加</button>
							</div>
							...
							<table class="table table-striped table-hover table-bordered table-condensed">
								...
								<tbody>
									...
									<tr>
										...
										<td><c:if test="${struct.DATA_PID eq '1'}">上部结构</c:if><c:if test="${struct.DATA_PID eq '2'}">下部结构</c:if><c:if test="${struct.DATA_PID eq '3'}">桥面系</c:if></td>
										<td>
											<div class="btn-group-sm">
												<button type="button" class="btn btn-warning btn-xs" onclick="javascript:upd('struct/updatePage','${struct.STRUCT_ID}','2');"><span class="glyphicon glyphicon-edit"></span> 修改</button>
												<button type="button" class="btn btn-danger btn-xs" onclick="javascript:del('struct/update','${struct.STRUCT_ID}','3');"><span class="glyphicon glyphicon-trash"></span> 删除</button>
											</div>
										</td>
									</tr>
									</c:forEach>
